...
export function ContentNote() {
  const { uiLanguage, game, acceptContentNote, declineContentNote } = useStore();
  if (!game) return null;
  ...
  return (
    <div className="h-screen flex items-center justify-center p-3 relative">
      ...
      <div className="ascii-box p-6 max-w-lg w-full relative z-10">
        <div className="text-xs text-gray-500 mb-3">{t(uiLanguage, 'contentNoteTitle')}</div>
